...
const routeMeta: Record<Route | 'demo', { title: string; description: string; path: string }> = {
  ...
  demo: { title: 'Demo — Backup Coverage Ledger', description: 'Try an isolated sample backup ledger with current, due, missing, unproven, and expired restore proof.', path: '/?demo=1' },
...
function ledgerHero(): string {
  if (isDemo) return `<section class="demo-intro" aria-labelledby="page-title"><p class="eyebrow"><span></span>Isolated sample ledger</p><h1 id="page-title">Review a sample backup ledger</h1><p>Five sample assets show current, due, unproven, expired, and missing restore proof.</p></section>`;
  return `<section class="hero ${records.length ? 'hero--compact' : ''}" aria-labelledby="page-title"><div class="hero-copy"><p class="eyebrow"><span></span>Backup coverage ledger</p><h1 id="page-title">Track backup coverage and restore tests</h1><p class="lede">For small IT teams that need one record of each critical asset, its backup, owner, and latest restore test.</p><div class="hero-actions"><a class="button button--primary" href="/?demo=1" data-route>Try it with sample data</a><span class="action-note">Opens an isolated example ledger.</span></div><ul class="hero-facts" aria-label="Product facts"><li>Free</li><li>Works offline after the first visit</li><li>Stored in this browser</li></ul></div><figure class="hero-art"><picture><source type="image/avif" srcset="/assets/proof-lattice-480.avif 480w, /assets/proof-lattice-960.avif 960w" sizes="(max-width: 700px) 100vw, 48vw"><source type="image/webp" srcset="/assets/proof-lattice-480.webp 480w, /assets/proof-lattice-960.webp 960w" sizes="(max-width: 700px) 100vw, 48vw"><img src="/assets/proof-lattice-960.jpg" width="960" height="640" ${records.length ? 'loading="lazy"' : 'fetchpriority="high"'} decoding="async" alt="Paper geometry links backup folders to restore-proof rings; one red line stops before its ring."></picture><figcaption><span aria-hidden="true">○—●</span> A listed backup is not restore proof. Record proof after someone restores and opens representative data.</figcaption></figure></section>`;
}
...
function recordList(filtered: LedgerRecord[]): string {
  if (!records.length) return `<div class="empty-state"><div class="empty-geometry" aria-hidden="true"><i></i><i></i><i></i></div><h3>No backup records yet</h3><p>Add the critical data set you would miss first. You can also import a CSV or flat YAML file.</p><button class="button button--primary" type="button" data-action="add">${icon('plus')} Add first asset</button></div>`;
